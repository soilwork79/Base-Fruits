...
class GameState {
    ...
    trailPool: ObjectPool<Trail>;
    scorePopupPool: ObjectPool<ScorePopup>;
    fruitHalfPool: ObjectPool<FruitHalf>;
    ...
    dirtyRegions: {x: number, y: number, w: number, h: number}[] = [];
...
class FruitSliceGame {
    ...
    checkFruitCollisions(x: number, y: number) {
        for (const fruit of this.state.fruits) {
            if (fruit.sliced || !fruit.active) continue;
            if (this.state.slicedThisSwipe.includes(fruit)) continue;
    ...
    processCombo() {
        ...
        if (comboCount >= 2) {
            ...
            if (comboCount >= 3) this.playSoundSafe(this.state.excellentSound);
            if (comboCount >= 5) this.playSoundSafe(this.state.amazingSound);
            if (comboCount >= 7) this.playSoundSafe(this.state.legendarySound);
        } else {
    ...
    showScorePopup(x: number, y: number, score: number, comboCount: number) {
        if (this.state.scorePopups.length >= MAX_SCORE_POPUPS) {
            const oldest = this.state.scorePopups.shift();
            if (oldest) this.state.scorePopupPool.release(oldest);
        }
    ...
    renderSmoothTrail(ctx: CanvasRenderingContext2D, points: Point[], opacity: number) {
    ...
    renderFruit(ctx: CanvasRenderingContext2D, fruit: Fruit) {
        ctx.save();
    ...
    renderFruitHalf(ctx: CanvasRenderingContext2D, half: FruitHalf) {
        ctx.save();
    ...
    renderScorePopup(ctx: CanvasRenderingContext2D, popup: ScorePopup) {
        ctx.save();
    ...
    launchFruit(isBomb: boolean = false) {
        ...
        if (isBomb) {
            const fuseAudio = this.state.fuseSound.cloneNode() as HTMLAudioElement;
    ...
    playSoundSafe(audio: HTMLAudioElement) {
        try {
    ...
    showGameOver() {
        ...
        if (gameOverScreen && finalScoreElement) {
            ...
            (window as any).currentScore = this.state.score;
        }
    }
...
// Blockchain ve Leaderboard fonksiyonları (değişiklik yok)
...
async function connectWallet() {
    ...
    try {
        ...
        if ((window as any).sdk) {
            ...
            rawProvider = (window as any).sdk.wallet.ethProvider;
...
async function saveScore(walletAddress: string, signer: any, rawProvider: any) {
    const btn = document.getElementById('save-score-button') as HTMLButtonElement;
    ...
    try {
        ...
        if ((window as any).sdk?.context?.user) {
